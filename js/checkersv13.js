...
function smartMove()
{
    ...
    for(i = 0; i < moves.length; i++)
    {
        temp = canJump(document.getElementById(moves[i].id));
        if(temp)
        {
            vitalMove.push(moves[i]);
            console.log(moves[i].id + " Can make a jump, " + vitalMove.length);
        }
...
function canJump(check)
{
    var Xcheck = parseInt((check.id).substring(1,2));
    var Xmove = 0;
    
    document.getElementById("AutoplayMode").textContent = 1;
    var i = 0;
    var moves = Reaction(check);
    moves = document.getElementsByClassName("Choice");
    
    for(i = 0; i < moves.length; i++)
    {
        Xmove = parseInt((moves[i].id).substring(1,2));
